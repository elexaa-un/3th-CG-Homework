...
function formViewMatrix(){	
    var eye=vec3(eyeRadius * Math.sin(toRad(eyePhi)) * Math.sin(toRad(eyeTheta)), 
                 eyeRadius * Math.cos(toRad(eyePhi)), 
                 eyeRadius * Math.sin(toRad(eyePhi)) * Math.cos(toRad(eyeTheta)));	
	eyePos = eye;
	var up=vec3(Math.sin(toRad(eyePhi-90)) * Math.sin(toRad(eyeTheta)), 
				 Math.cos(toRad(eyePhi-90)), 
				 Math.sin(toRad(eyePhi-90)) * Math.cos(toRad(eyeTheta)));	
	//用LOOKAT函数生成观察变换矩阵
